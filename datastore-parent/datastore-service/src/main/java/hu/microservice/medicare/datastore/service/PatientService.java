...
public class PatientService {
	...
	public PatientData create(PatientData patientData) {
		return mapper.map(repository.save(mapper.map(patientData)));
	}
	...
	private void update(PatientDataEntity entity, PatientData dto) {
		entity.setActiveSportActivities(dto.getActiveSportActivities().stream().map(sportMapper::map).collect(Collectors.toSet()));
		entity.setAlcoholRegularity(dto.getAlcoholRegularity());
		entity.setAncestorDeathCauses(dto.getAncestorDeathCauses().stream().map(illnessMapper::map).collect(Collectors.toSet()));
